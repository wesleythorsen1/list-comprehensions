...
exports.composite = new Benchmark.Suite("composite")
  ...
  .add("Enumerable (extension)", function () {
    const result = data.where(isEven).select(square).select(multiply).where(
      isEven,
    ).toArray();
  })
  .add("Lodash", function () {
    const result = _.filter(data, isEven).map(square).map(multiply).filter(
      isEven,
    );
  })
  .add("Underscore", function () {
    const result = underscore.filter(data, isEven).map(square).map(multiply)
      .filter(isEven);
  });
