...
Deno.test("it should count each unique element when using identity as keySelector", () => {
  // arrange
...
Deno.test("it should use a custom comparer to group objects correctly", () => {
  // arrange
  interface Item {
    id: number;
  }
  const data = Enumerable.from<Item>([{ id: 1 }, { id: 2 }, { id: 1 }, {
    id: 3,
  }]);
