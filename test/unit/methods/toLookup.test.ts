...
describe('toLookup', () => {
  it('should return an empty map when the source is empty', () => {
    // arrange
  ...
  it('should group each element individually when using identity as keySelector', () => {
    // arrange
  ...
  it('should apply the valueSelector to transform grouped values', () => {
    // arrange
    interface Person {
      id: number;
      name: string;
    ...
    // assert
    expect(lookup.size).toBe(2);
    expect(lookup.get(1) ? [...lookup.get(1)!] : []).toEqual(['Alice', 'Charlie']);
